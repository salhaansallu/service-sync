                                ...
                                @if ($repairs && $repairs->count() > 0)
                                    @foreach ($repairs as $item)
                                        <tr id="category{{ $item->id }}">
                                            ...
                                            <td class="text-start">{{ currency($item->total-$item->cost) }}</td>
                                            <td class="text-start">{{ getCustomer($item->customer)->name }}</td>
                                            <td class="text-start">{{ date('d-m-Y', strtotime($item->created_at)) }}</td>
                                            @if (hasDelivery($item->bill_no))
                                                <td
                                                    class="text-start @if (getDeliveryStatus($item->bill_no) == 'Delivered') bg-primary @elseif(getDeliveryStatus($item->bill_no) == 'Return') bg-warning @else bg-danger @endif">
                                                    Delivery ({{ getDeliveryStatus($item->bill_no) }})</td>
                                            @else
                                                <td class="text-start bg-primary">
                                                    {{ $item->status }}</td>
                                            @endif
                                            <td class="text-start">
                                                <div class="d-flex align-items-center list-action justify-content-start">
                                                    <a class="badge bg-primary mr-2" data-toggle="tooltip"
                                                        ...
                                                        href="/dashboard/bill/edit/{{ $item->id }}"><i
