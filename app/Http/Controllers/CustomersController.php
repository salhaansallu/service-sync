        ...
        if ($request->has('code') && preg_match('/^\d{4}$/', sanitize($request->input('code')))) {
            ...
            if ($sessionData) {
                ...
                if (Hash::check($code, $sessionData->otp)) {
                    ...
                    if ($data->count() > 0) {
                        $repairs = Repairs::where('customer', $data[0]->id)->orderBy('id', 'DESC')->get(['bill_no', 'model_no', 'serial_no', 'fault', 'advance', 'total', 'delivery', 'status', 'invoice', 'created_at', 'paid_at', 'warranty']);
