...
class RepairsController extends Controller
{
    ...
    public function downloadDetailedReport(Request $request)
    {
        ...
        $fileName = 'sales-detailed-report-' . date('Y-m-d-H-i-s') . '.csv';
        $report = generateDetailedSalesCsvReport($sales, $fromDate, $toDate, $fileName);

        if (!$report->generated) {
            return response()->json([
                'error' => 1,
                'msg' => 'Unable to generate CSV report',
            ], 500);
        }
